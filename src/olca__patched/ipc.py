...
class Client(object):
    ...
    def insert(self, model: E):
        ...
        if model is None:
            return
        json = model.to_json()
        resp, err = self.__post('data/put', json)
        if err:
    ...
    def delete(self, model: E):
        ...
        resp, err = self.__post('data/delete', json)
        if err:
            log.error('failed to delete model: %s', err)
            return err
        return resp

    def calculate(self, setup: schema.CalculationSetup) -> Tuple[Any, Optional[str]]:
        """
        ...
        ```
        """
        resp, err = self.__post('result/calculate', setup.to_json())
        print(resp, flush=True)
        if err:
            log.error('calculation failed: %s', err)
            return resp, err
        while resp.get('isReady') == False:
            time.sleep(2)
            resp, err = self.__post('result/state', resp)
            print(resp, flush=True)
            if err:
                log.error('calculation failed: %s', err)
                return resp, err
    ...
    def simulator(self, setup: schema.CalculationSetup) -> schema.Ref:
        ...
        ```
        """
        resp, err = self.__post('result/simulate', setup.to_json())
        if err:
    ...
    def next_simulation(self, simulator: schema.Ref) -> schema.SimpleResult:
        ...
        resp, err = self.__post('result/simulate/next', simulator.to_json())
        print(resp, flush=True)
        if err:
            log.error('next_simulation failed: %s', err)
            return resp, err
        while resp.get('isReady') == False:
            time.sleep(2)
            resp, err = self.__post('result/state', resp)
    ...
    def get_descriptors(self, model_type: ModelType) -> Iterator[schema.Ref]:
        ...
        result, err = self.__post('data/get/descriptors', params)
        if err:
    ...
    def get(self, model_type: Type[E],
        ...
        if name != '':
            params['name'] = name
        result, err = self.__post('data/get', params)
        if err:
    ...
    def get_all(self, model_type: Type[E]) -> Iterator[E]:
        ...
        result, err = self.__post('data/get/all', params)
        if err:
    ...
    def get_providers_of(self, flow: Union[schema.Ref, schema.Flow]) \
        ...
        providers, err = self.__post('data/get/providers', params)
        if err:
    ...
    def excel_export(self, result: schema.SimpleResult, path: str):
        ...
        _, err = self.__post('export/json-ld', params)
        if err:
            log.error('Excel export to %s failed: %s', path, err)

    def json_export_result_detail(self, result_specifier: str, params: dict, directory_path: str, file_prefix: str):
        """
        Export the result detail specified in a JSON file, with the given file_prefix, into the
        directory specified by directory_path.  It is assumed that calculate() was previously called and the
        calculation ran to completion, yielding a result_state ID.

        :param result_specifier: One of the strings listed below.
        :param params: The JSON params for the call, see below.
        :param directory_path: The path of the directory to contain the JSON file.
        :param file_prefix: The prefix for the JSON file, the suffix is built from the result_specifier.

        This is the list of sample callsclient.json_export_result_detail:

          client.json_export_result_detail("result/demand", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/tech-flows", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/envi-flows", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/impact-categories", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-requirements", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-requirements-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/scaling-factors", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/totality-factors", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/scaled-tech-flows-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/unscaled-tech-flows-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/totality-factor-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-flows", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-flow-value-of", {"@id": result_id, "enviFlow": envi_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/flow-contributions-of", {"@id": result_id, "enviFlow": envi_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/direct-interventions-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/direct-intervention-of", {"@id": result_id, "enviFlow": envi_flow, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/flow-intensities-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/flow-intensity-of", {"@id": result_id, "enviFlow": envi_flow, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-interventions-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-intervention-of", {"@id": result_id, "enviFlow": envi_flow, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impacts", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impact-value-of", {"@id": result_id, "impactCategory": impact_category}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impacts/normalized", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impacts/weighted", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/impact-contributions-of", {"@id": result_id, "impactCategory": impact_category}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/direct-impacts-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/direct-impact-of", {"@id": result_id, "impactCategory": impact_category, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impacts-of-one", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/impact-intensity-of", {"@id": result_id, "impactCategory": impact_category, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impacts-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-impact-of", {"@id": result_id, "impactCategory": impact_category, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/impact-factors-of", {"@id": result_id, "impactCategory": impact_category}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/impact-factor-of", {"@id": result_id, "impactCategory": impact_category, "enviFlow": envi_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/flow-impacts-of", {"@id": result_id, "impactCategory": impact_category}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/flow-impact-of", {"@id": result_id, "impactCategory": impact_category, "enviFlow": envi_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-costs", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/cost-contributions", {"@id": result_id}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/direct-costs-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/total-costs-of", {"@id": result_id, "techFlow": tech_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/upstream-of-flow", {"@id": result_id, "path": path, "enviFlow": envi_flow}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/upstream-of-impact-category", {"@id": result_id, "path": path, "impactCategory": impact_category}, OUTPUT_DIR, curr_production_process_name)
          client.json_export_result_detail("result/upstream-of-costs", {"@id": result_id, "path": path}, OUTPUT_DIR, curr_production_process_name)

          Note that path is a string which could be:
            - "/" or
            - "<provider1_id>::<flow1_id>/<provider2_id>::<flow2_id>/..."
        """
        suffix = "-" + result_specifier.replace("/", "-")
        file_name = f"{file_prefix}{suffix}.json"
        file_path = os.path.join(directory_path, file_name)
        file = open(file_path, "w")

        (data, err) = self.__post(result_specifier, params)
        if data is not None:
            file.write(json.dumps(data, indent=4))
        else:
            file.write(json.dumps({result_specifier: err}, indent=4))
        file.close()
        return data, err

    def go_farther_upstream(self, tech_flow_list, params, depth, max_depth):
        depth += 1
        if depth > max_depth:
            return
        ...
        for tech_flow in tech_flow_list:
            tech_flow_kernel = tech_flow.get("techFlow")
            if tech_flow_kernel is None:
                return
            result = tech_flow.get("result")
            provider = tech_flow_kernel.get("provider")
            if provider is None:
                return
            provider_id = provider["@id"]
            provider_name = provider["name"]
            flow = tech_flow_kernel.get("flow")
            if flow is None:
                return
            flow_id = flow["@id"]
            flow_name = flow["name"]
            path = provider_id + "::" + flow_id
            params["path"] = path

            print("{indent}{provider} :: {flow} : {result}".format(indent=indent, provider=provider_name, flow=flow_name, result=result), flush=True)
            # (sub_tech_flow_list, err) = self.__post("result/upstream-of-impact-category", params)
            (sub_tech_flow_list, err) = self.__post("result/upstream-impacts-of", params)
            if sub_tech_flow_list is not None:
                self.go_farther_upstream(sub_tech_flow_list, params, depth=depth, max_depth=MAX_DEPTH)
                tech_flow["upstreamTechFlows"] = sub_tech_flow_list
            else:
                tech_flow_list.append({"result/upstream-of-impact-category": err})

    def json_export_upstream_of_impact_category(self, impact_params: dict, directory_path: str, file_prefix: str):
        suffix = "-result-upstream-of-impact-category"
        file_name = f"{file_prefix}{suffix}.json"
        file_path = os.path.join(directory_path, file_name)
        file = open(file_path, "w")

        params = {}
        params["@id"] = impact_params["@id"]

        upstream_of = []
        for impact in impact_params["totalImpacts"]:
            params["path"] = "/"
            params["impactCategory"] = impact["impactCategory"]
            impact_category_name = impact["impactCategory"]["name"]
            amount = impact["amount"]
            print("------------------", flush=True)
            print("{name}: {amount}".format(name=impact_category_name, amount=amount), flush=True)
            # (tech_flow_list, err) = self.__post("result/upstream-of-impact-category", params)
            (tech_flow_list, err) = self.__post("result/upstream-impacts-of", params)
        ...
        file.write(json.dumps(upstream_of, indent=4))
        file.close()

    def json_export_flows_of_impact_category(self, impact_params: dict, directory_path: str, file_prefix: str):
        suffix = "-result-flows-of-impact-category"
        file_name = f"{file_prefix}{suffix}.json"
        file_path = os.path.join(directory_path, file_name)
        file = open(file_path, "w")
        ...
        for impact in impact_params["totalImpacts"]:
            ...
            print("------------------", flush=True)
            print("{name}: {amount}".format(name=impact_category_name, amount=amount), flush=True)
            (flow_list, err) = self.__post("result/flow-impacts-of", params)

            if flow_list is not None:
                flows_of.append({impact_category_name: flow_list})
            else:
                flows_of.append({"result/flows-of-impact-category": err})

        file.write(json.dumps(flows_of, indent=4))
        file.close()
    ...
    def dispose(self, params: dict):
        ...
        if params is None:
            return
        _, err = self.__post('result/dispose', params)
        if err:
            log.error('failed to dispose of object: %s', err)
    ...
    def create_product_system(self, process_id: str, default_providers='prefer',
        ...
        config = {
            'preferUnitProcesses': preferred_type,
            'providerLinking': default_providers,
        }
        r, err = self.__post('data/create/system', {
            'process': process,
            'config': config,
        })
        if err:
